...
idf_file_name = r"C:\Users\danie\OneDrive\Dokumenter\MasterThesis\Test\ReMoni_OS_Model_jan_reduced_sizing.idf"  # building energy model (BEM) IDF file
# Weather Path
...
cvs_output_path = r'C:\Users\danie\OneDrive\Dokumenter\MasterThesis\Test\One_timestep_output.csv'
...
# Should be same varialbes as obersevation_function
    ...
    #   self.zn2_temp = None  # deg C                       # self nr. 5
        ...
        self.first_value_recorded = False
    ...
    def observation_function(self):
        ...
        # print(self.bca.get_ems_data(['t_hours']))
        # self.time = self.bca.get_ems_data(['t_hours'])
        #check that self.time is less than current time
        time_data = self.bca.get_ems_data(['t_hours'])

        if isinstance(time_data, list):
            self.time_of_day = time_data[0]
        else:
            self.time_of_day = time_data

        if not self.first_value_recorded:
            ...
            self.fan_mass_flow = var_data[4]  # Get the fan mass flow value

            # Write the value to the CSV file
            with open(cvs_output_path, 'w') as f:
                f.write(f"Time, Fan Mass Flow Rate\n")
                f.write(f"{self.time_of_day}, {self.fan_mass_flow}\n")

            # Print the value and set the flag
            print(f"Recorded first fan_mass_flow: {self.fan_mass_flow} at time {self.time_of_day}")
            self.first_value_recorded = True  # Set the flag to True

        # Increment the timestep counter
        self.timestep_counter += 1


        if self.timestep_counter >= 1:
                print("Stopping simulation after the first timestep.")
                self.simulation_should_stop = True
    

        current_time = datetime.datetime.now()
        simulation_time = datetime.datetime.combine(current_time.date(), datetime.time(int(self.time_of_day)))

        if simulation_time < current_time:
        # if self.time_of_day < datetime.datetime.now():
            # Get data from simulation at current timestep (and calling point) using ToC names
            var_data = self.bca.get_ems_data(list(self.bca.tc_var.keys()))
        ...
        if self.simulation_should_stop:
            print("Simulation terminated after one timestep.")
            return            
...
# Define thresholds - Can find example in model_test.py.
    ...
    def act(self, state):
           co2_indoor_con = state[7]

           if co2_indoor_con > 0.9:
                action = 10          #Fan speed off
           elif 0.7 <= co2_indoor_con < 0.9:
               action = 5          # Medium fan speed
           else:
