...
# Should be same varialbes as obersevation_function
    ...
    def observation_function(self):
        ...
        if not self.first_value_recorded:
            ...
            # Save the value to the CSV file
            with open(cvs_output_path, 'w') as f:
                f.write(f"Time, Fan Mass Flow Rate\n")
                f.write(f"{self.time}, {self.fan_mass_flow}\n")

            # Stop the simulation after recording the first value
            print(f"Recorded first fan_mass_flow: {self.fan_mass_flow} at time {self.time}")
            self.first_value_recorded = True  # Set the flag
            exit()
